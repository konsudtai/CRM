...
```
Phase 1: CRM Stack
  [1/8] Deploy CloudFormation (VPC, RDS, RDS Proxy, Lambda, API GW, S3, CloudFront)
  [2/8] Get stack outputs (CloudFront URL, API URL, DB endpoint)
  [3/8] Generate seed.sql with admin credentials (bcrypt hash)
  [4/8] Upload frontend to S3 with correct content types
  [5/8] Invalidate CloudFront cache
  [5.5/8] Auto-init database (if psql available)

Phase 2: AI Stack
  [6/8] Deploy AI CloudFormation (S3 KB bucket, IAM roles)
  [7/8] Get AI outputs (KB bucket name)
  [8/8] Upload sample Knowledge Base documents
```

### Post-Deploy Step 1: Subscribe CloudFront Pro Plan ($15/mo)

> CloudFront Pro Plan **ไม่สามารถ auto-subscribe ผ่าน CLI/API/CloudFormation ได้** ต้องทำผ่าน Console เท่านั้น (ณ พ.ค. 2026)

```bash
# ดู Distribution ID ที่สร้างจาก deploy.sh
aws cloudfront list-distributions --query "DistributionList.Items[?Comment=='SalesFAST 7 prod'].{Id:Id,Domain:DomainName,Status:Status}" --output table
```

**วิธี Subscribe (ทำครั้งเดียว ~2 นาที):**

```
1. เปิด https://console.aws.amazon.com/cloudfront/v4/home
2. คลิก "Distributions" ในเมนูซ้าย
3. คลิกเลือก Distribution ที่มี Comment "SalesFAST 7 prod"
4. คลิก "Edit" หรือ "Pricing plan" tab
5. เลือก "Pro" ($15/month)
6. กด "Save changes"
7. เสร็จ — WAF + DDoS + DNS + Logs + 50GB S3 เปิดใช้งานทันที
```

> Pro plan คิดเงินแบบ pro-rate ถ้า subscribe กลางเดือน เช่น วันที่ 16 ของเดือน 30 วัน จะคิด $7.50 สำหรับ 15 วันที่เหลือ เดือนถัดไปคิดเต็ม $15

### Post-Deploy Step 2: Initialize Database

Deploy script จะพยายาม auto-init ถ้ามี `psql` แต่ถ้า RDS อยู่ใน private subnet (ปกติจะเป็นแบบนี้) ต้อง init เอง:

#### วิธี A: RDS Query Editor (ง่ายที่สุด — แนะนำ)

```
1. เปิด AWS Console > RDS > Query Editor
2. เลือก Database: sf7-prod
3. Username: salesfast7
4. Password: (db-pass ที่ใส่ตอน deploy หรือที่ auto-generate)
5. Database: salesfast7
6. กด "Connect"

7. Copy เนื้อหาจาก database/schema.sql วางใน editor แล้วกด "Run"
   (รอ ~10 วินาที — สร้าง 30+ tables)

8. Copy เนื้อหาจาก /tmp/sf7-seed-generated.sql วางใน editor แล้วกด "Run"
   (สร้าง tenant + admin user + roles + permissions + pipeline stages)

9. ตรวจสอบ: Run query "SELECT email FROM users;" → ต้องเห็น admin email
```

#### วิธี B: EC2 Bastion Host (ถ้ามี EC2 ใน VPC เดียวกัน)

```bash
# SSH เข้า EC2 ที่อยู่ใน VPC เดียวกับ RDS
ssh ec2-user@<BASTION_IP>

# Install psql (ถ้ายังไม่มี)
...
#### วิธี C: Lambda DB Init (automated — สำหรับ advanced users)

```bash
# Upload SQL files ไป S3
aws s3 cp ../database/schema.sql s3://sf7-prod-files-<ACCOUNT_ID>/db-init/
aws s3 cp /tmp/sf7-seed-generated.sql s3://sf7-prod-files-<ACCOUNT_ID>/db-init/

# สร้าง Lambda function ที่อ่าน SQL จาก S3 แล้ว execute กับ RDS
# (ใช้ Lambda ที่อยู่ใน VPC เดียวกัน — มี access ถึง RDS ได้)
```

#### วิธี D: CloudShell + psql (ถ้า RDS เป็น publicly accessible)

```bash
# ⚠️ ใช้ได้เฉพาะกรณี RDS ตั้ง PubliclyAccessible: true (ไม่แนะนำสำหรับ production)
# Default ของ SalesFAST 7 คือ PubliclyAccessible: false (ปลอดภัยกว่า)

# ถ้าต้องการใช้วิธีนี้ ต้องแก้ cloudformation.yaml:
#   PubliclyAccessible: true
#   และเพิ่ม Security Group rule ให้ CloudShell IP เข้าถึง port 5432
...
### Post-Deploy Step 3: ทดสอบระบบ

```bash
# 1. เปิด CloudFront URL ใน browser
echo "Open: https://<CLOUDFRONT_DOMAIN>"

# 2. Login ด้วย admin credentials ที่ตั้งตอน deploy
#    Email:    (--email ที่ใส่)
#    Password: (--password ที่ใส่)

# 3. ระบบจะบังคับเปลี่ยนรหัสผ่านครั้งแรก (force_password_change: true)

# 4. ตรวจสอบหน้าต่างๆ:
#    - Dashboard: ดู KPI cards, pipeline chart
#    - Accounts: สร้าง/ดูบัญชีลูกค้า
#    - Pipeline: ลาก Lead ผ่าน stages
#    - Quotations: สร้างใบเสนอราคา
#    - Settings: จัดการ Users, Roles (Admin only)
#    - น้องขายไว: คลิก floating widget มุมขวาล่าง
```
