...
export async function handleLineWebhook(c: Context) {
  ...
  // Fargate: process async via setImmediate (fire-and-forget in-process)
  // No Lambda self-invoke needed — Fargate is long-running.
  for (const event of events) {
    ...
    // Schedule async — respond to LINE within 1s, process in background
    setImmediate(() => {
      processLineAsync(payload).catch((e: any) => {
        console.error('[LINE] processAsync error:', e?.message);
      });
    });
  }
