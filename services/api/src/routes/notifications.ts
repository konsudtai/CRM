...
notifications.get('/', authMiddleware, async (c) => {
  ...
  // Admin and Sales Manager see all notifications for their tenant
  let r;
  if (userRole === 'Admin' || userRole === 'Sales Manager') {
    r = await query(t, 'SELECT * FROM notifications WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2', [t, limit]);
  } else {
    r = await query(t, 'SELECT * FROM notifications WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2', [userId, limit]);
  }
...
notifications.patch('/:id/read', authMiddleware, async (c) => {
  ...
  await query(t, `UPDATE notifications SET status = 'read', read_at = NOW() WHERE id = $1`, [id]);
  return c.json({ ok: true });
});
